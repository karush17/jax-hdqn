...
def main(_):
    ...
    epsilon_by_frame = lambda frame_idx: FLAGS.epsilon_final + (FLAGS.epsilon_start\
                     - FLAGS.epsilon_final) * math.exp(-1. * frame_idx / FLAGS.epsilon_decay)
    ...
    while frame_idx < FLAGS.steps:
        ...
        while not done and goal != np.argmax(state):
            ...
            if len(replay_buffer) >= FLAGS.batch_size:
                agent.model = update(agent.model, replay_buffer, FLAGS.batch_size)
            if len(meta_replay_buffer) >= FLAGS.batch_size:
                agent.meta_model = meta_update(agent.meta_model, meta_replay_buffer, FLAGS.batch_size)
            frame_idx += 1
